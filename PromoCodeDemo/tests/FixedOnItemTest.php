...
class FixedOnItemTest extends WebTestCase
{
    ...
    public function testApply()
    {
        ...
        $service = $this->getMockBuilder('Bundle\PublicSiteBundle\Component\Cart\TotalsGenerator\PromoCode\FixedOnItem')
    ...
    public function testCalculateQtyOfMatchingCatalogItem()
    {
        $file = file_get_contents('../Data/promocode_test_1.json');
        $normalizedLines = json_decode($file, true);
        ...
        $service->linesHaveMinimumSubtotal($normalizedLines);
        $service->calculateQtyOfMatchingCatalogItem($normalizedLines);
        $qty = $service->qtyOfMatchingCatalogItem;

        $this->assertEquals(3, $qty);
    }

    public function testGetFixedOfItemAmount()
    {
        $file = file_get_contents('../Data/promocode_test_1.json');
        $normalizedLines = json_decode($file, true);
        ...
        $service->linesHaveMinimumSubtotal($normalizedLines);
        $service->calculateQtyOfMatchingCatalogItem($normalizedLines);
        $result = $service->getFixedOnItemAmount();
